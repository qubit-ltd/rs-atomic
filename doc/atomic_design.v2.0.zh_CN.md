...
6. **简洁性**：API 表面积小，仅为整数读-改-写操作提供有限的 `_with_ordering` 变体
...
| 操作 | 内存序 | 使用场景 | 决策理由 |
|------|--------|---------|---------|
| `load()` | `Acquire` | 读取共享状态 | 确保能看到其他线程的最新写入 |
| `store(value)` | `Release` | 更新共享状态 | 确保之前的写入对其他线程可见 |
...
```rust
// 场景1：共享配置（需要同步）
config.store(new_config);           // Release：确保配置数据可见
let current = config.load();       // Acquire：读取最新配置

// 场景2：标志位通信
data.write(value);                // 普通写入
flag.store(true);                   // Release：确保 data 写入可见

// 另一线程
if flag.load() {                   // Acquire：建立 happens-before
...
fn handle_request() {
    REQUEST_COUNT.fetch_inc();  // Relaxed：性能最优

    if let Err(_) = process() {
        ERROR_COUNT.fetch_inc();  // Relaxed：不需要同步
    }
}
...
// 线程1
data.push(42);
counter.fetch_inc();  // Relaxed：不能保证 data 可见！

// 线程2
if counter.load() > 0 {  // Acquire：无法建立 happens-before
...
// 线程1
data.push(42);
ready.store(true);  // Release：确保 data 可见

// 线程2
if ready.load() {  // Acquire：建立 happens-before
...
2. **标志位变化通常伴随数据变化**：
   ...
   // 检查标志位后访问数据
   if flags.load() & READY_FLAG != 0 {  // 需要 Acquire
       data.read();
   }
   ```
...
// 线程2：启动
if state.load() & INITIALIZED != 0 {  // Acquire：看到 resources
...
fn on_connection_opened() {
    ...
    if old_peak < current {
        peak_time.store(now_millis());  // Release：时间戳可见
...
| 操作 | 内存序 | 使用场景 | 决策理由 |
|------|--------|---------|---------|
| `load()` | `Acquire` | 检查标志 | 确保能看到相关数据的修改 |
| `store(value)` | `Release` | 设置标志 | 确保数据修改对其他线程可见 |
...
// 生产者
shared_data = 42;
data_ready.store(true);  // Release：确保 shared_data 写入可见

// 消费者
if data_ready.load() {  // Acquire：建立 happens-before
...
`AtomicRef<T>` 使用 `arc_swap::ArcSwap<T>` 后端封装 `Arc<T>`，用于共享不可变数据的原子更新：

| 操作 | 内存序 | 使用场景 | 决策理由 |
|------|--------|---------|---------|
| `load()` | ArcSwap 默认语义 | 获取一个拥有所有权的 `Arc<T>` 克隆 | 当前引用的安全快照 |
| `load_guard()` | ArcSwap 默认语义 | 短生命周期读取 | 快路径上避免克隆 `Arc<T>` |
| `store(value)` | ArcSwap 默认语义 | 更新引用 | 发布新的共享值 |
...
fn reload_config() {
    ...
    config.store(new_config);  // 发布新配置
}

// 读取配置（在工作线程中）
fn process_request() {
    let cfg = config.load();  // 获取最新配置
    // 使用 cfg...
}
```
...
| 操作 | 内存序 | 使用场景 | 决策理由 |
|------|--------|---------|---------|
| `load()` | `Acquire` | 读取浮点值 | 标准读操作 |
| `store(value)` | `Release` | 设置浮点值 | 标准写操作 |
| `swap(value)` | `AcqRel` | 原子交换 | 标准读-改-写语义 |
| `fetch_add(delta)` | `AcqRel` / `Acquire` | 原子加法 | CAS 循环实现 |
| `fetch_sub(delta)` | `AcqRel` / `Acquire` | 原子减法 | CAS 循环实现 |
| `fetch_mul(factor)` | `AcqRel` / `Acquire` | 原子乘法 | CAS 循环实现 |
| `fetch_div(divisor)` | `AcqRel` / `Acquire` | 原子除法 | CAS 循环实现 |

**注意事项**：

1. **浮点运算通过 CAS 循环实现**，不是硬件原子指令
2. **高竞争场景性能较差**，适合低竞争的统计场景
3. **算术操作返回旧值**，保持 Rust `fetch_*` API 的语义一致性
...
2. 防止误用（用户可能不理解内存序）
3. 保持简洁性（符合"易用封装"的定位）
4. 为常见同步敏感计数器提供有限的整数读-改-写 `_with_ordering` 变体
5. `inner()` 仍然是高级用户的 escape hatch
...
**原因**：实现这些 trait 会隐藏读取操作，用户应该显式调用 `load()` 或 `inner().load()`。
...
// ✅ 明确的代码
if atomic1.load() == atomic2.load() {  // 清楚地表明这是两次独立的读取
    // ...
}
```
...
下表说明了引用类型方法使用的底层 `arc-swap` 操作：

| **API 方法** | **后端操作** | **说明** |
|-------------|-------------|---------|
| `new(value)` | `ArcSwap::from(value)` | 创建新的原子引用 |
| `load()` | `load_full()` | 以拥有所有权的 `Arc<T>` 加载当前引用 |
| `load_guard()` | `load()` | 以短生命周期 guard 加载当前引用 |
| `store(value)` | `store(value)` | 存储新引用 |
| `swap(value)` | `swap(value)` | 交换引用并返回旧引用 |
| `compare_set(current, new)` | `compare_and_swap(current, new)` + 指针比较 | CAS 操作，返回 `Result` |
| `compare_set_weak(current, new)` | 委托给 `compare_set` | 弱 CAS 形态的 API；当前后端不会额外引入虚假失败 |
...
for _ in 0..1000 {
    counter.fetch_add(1.0); // 每次都需要 CAS 循环
}
...
fn float_custom_update_example() {
    ...
    // 乘法操作
    temperature.fetch_mul(1.8); // 摄氏度转华氏度的一部分
    temperature.fetch_add(32.0); // 完成转换

    println!("华氏温度: {}", temperature.load());
}
...
| **基础操作** | `get()` | `load()` | ✅ | 读取当前值 |
| | `set(int newValue)` | `store(value: i32)` | ✅ | 设置新值 |
...
| | `incrementAndGet()` | - | ❌ | 不暴露返回新值的变体；必要时由 `fetch_inc()` 推导 |
| | `getAndDecrement()` | `fetch_dec()` | ✅ | 自减，返回旧值 |
| | `decrementAndGet()` | - | ❌ | 不暴露返回新值的变体；必要时由 `fetch_dec()` 推导 |
| **算术操作** | `getAndAdd(int delta)` | `fetch_add(delta: i32)` | ✅ | 加法，返回旧值 |
| | `addAndGet(int delta)` | - | ❌ | 不暴露返回新值的变体；必要时由 `fetch_add()` 推导 |
| | - | `fetch_sub(delta: i32)` | ✅ | 减法，返回旧值（Rust 特有）|
...
| **最大/最小值** | - | `fetch_max(value)` | ✅ | 取最大值，返回旧值（Rust 特有）|
| | - | `fetch_min(value)` | ✅ | 取最小值，返回旧值（Rust 特有）|
| **类型转换** | `intValue()` | `load()` | ✅ | 直接用 `load()` |
| | `longValue()` | `load() as i64` | ✅ | 通过 as 转换 |
| | `floatValue()` | `load() as f32` | ✅ | 通过 as 转换 |
| | `doubleValue()` | `load() as f64` | ✅ | 通过 as 转换 |
...
| **基础操作** | `get()` | `load()` | ✅ | 读取当前值 |
| | `set(boolean newValue)` | `store(value: bool)` | ✅ | 设置新值 |
...
| **基础操作** | `get()` | `load()` | ✅ | 获取当前引用 |
| | `set(V newValue)` | `store(value: Arc<T>)` | ✅ | 设置新引用 |
| | `lazySet(V newValue)` | - | ❌ | 不单独暴露 lazy-set API；使用 `store()` |
...
| **弱 CAS** | `weakCompareAndSet` | `compare_set_weak` | 两者等价 |
...
| **API 数量** | ~20 个方法/类型 | ~25 个方法/类型 | Rust 只暴露有限的 `_with_ordering` 变体，不镜像每个方法 |
...
```rust
// 使用 compare_set（需要在失败时从 Err 中提取值）
let mut current = atomic.load();
loop {
...
1. **`compare_set`**：封装标准库的 `compare_exchange`，返回 `Result<(), T>`
...
|---------|--------------|---------|---------|
| **读取** (`load()`) | `Acquire` | 轻量级，读屏障 | 读取共享状态 |
| **写入** (`store()`) | `Release` | 轻量级，写屏障 | 更新共享状态 |
...
impl SpinLock {
    ...
    fn unlock(&self) {
        ...
        // ✅ 正确：释放锁必须用 Release
        self.locked.store(false);  // 默认 Release
    }
}
```
...
// 线程 1
data = 100;
flag.store(true);  // Release - 保证 data 的写入可见

// 线程 2
if flag.load() {  // Acquire - 保证看到 data 的更新
...
fn update_stats(&self) {
    ...
    self.timestamp.store(now());         // 已经是 Release
}
```
...
flag.store(true); // Release - 保证之前的写入可见
if flag.load() { // Acquire - 保证读取到最新值
...
let atomic = AtomicI32::new(0);
let value = atomic.load();                // 自动 Acquire
atomic.store(42);                          // 自动 Release
...
- ✅ `AtomicI128`, `AtomicU128` - 128位整数（通过 `portable-atomic` 实现）
