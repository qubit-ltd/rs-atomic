...
Qubit Atomic 是一个全面的原子操作库，提供易于使用的原子类型和合理的默认内存序，类似于 Java 的 `java.util.concurrent.atomic` 包。它隐藏了内存序的复杂性，让基础类型的转发封装保持轻量，并允许高级用户访问底层类型以进行细粒度控制。
...
- **128 位可移植性**：`Atomic<i128>` 和 `Atomic<u128>` 使用
  `portable-atomic`；是否原生 lock-free 取决于目标平台，在缺少合适原子指令的目标上，
  fallback 可能使用锁
...
- **函数式更新**：`fetch_update`、`update_and_get`、`try_update`、`try_update_and_get`、`fetch_accumulate`、`accumulate_and_get`
- **const 初始化出口**：`atomic::primitive::AtomicBool`、`AtomicU8`、`AtomicF32` 等具体封装提供 `const fn new`，用于泛型 `Atomic<T>` 不能用于 const 上下文的静态初始化场景
...
- **函数式更新**：用 `fetch_update` / `update_and_get` 原子地转换引用，或用 `try_update` / `try_update_and_get` 条件更新
...
### 🎯 **聚焦的公开 API**
- **Atomic<T>**：基础原子值的泛型入口
- **`atomic::primitive::*`**：用于 `const fn new` 场景的具体基础类型封装
- **AtomicRef<T>**：原子的 `Arc<T>` 引用封装
- **显式引用 fork**：`AtomicRef::fork()` 把当前观察到的 `Arc<T>` 放入独立的原子容器
...
`_with_ordering` 变体仅用于整数读-改-写计数加减操作，适合计数值同时作为同步信号的场景。
...
| 操作类型 | 默认内存序 | 原因 |
|---------|-----------|------|
| **纯读操作** (`load()`) | `Acquire` | 当读取观察到 release sequence 时，后续操作可观察该 release 之前发布的数据；不保证读到全局最新值 |
| **纯写操作** (`store()`) | `Release` | 向观察到该 store 或其 release sequence 的 acquire 操作发布先前写入 |
| **读-改-写操作** (`swap()`、CAS) | `AcqRel` | 从观察到的 release 获取数据，并通过成功修改发布先前写入 |
...
当整数计数加减需要同步语义时，优先使用 ordered 变体，再考虑访问
`inner()`：
...
| **可空性** | 允许 `null` | `AtomicRef<T>` 不提供 | 需要原子可空引用时使用 `arc_swap::ArcSwapOption<T>` 或其他同步类型 |
...
基础类型封装使用 `#[repr(transparent)]` 和 `#[inline]`。在优化构建中，简单转发方法
旨在编译为对应的底层原子操作：
...
这个范围不包括本身就有额外工作的操作：浮点与 checked count 更新使用 CAS 重试循环，
`AtomicRef<T>` 则保留 `ArcSwap` 的引用计数、回收和重试成本。应在目标工作负载上测量，
而不是预设机器码完全相同。

### 何时使用 `inner()`

当默认内存序和操作语义满足需要时，使用默认 API。仅在以下情况使用 `inner()`：
- 聚焦的 wrapper API 没有暴露所需内存序
- 经测量确认的热点可从另一种合法内存序中获益
- 底层算法需要精确控制后端
