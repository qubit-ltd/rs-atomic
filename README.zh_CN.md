...
### 🔢 **同步计数器类型**
- **AtomicCounter**：用于活跃任务数、进行中请求数、资源使用数的非负计数器
- **AtomicSignedCounter**：用于差值、余额、积压量、偏移量的有符号计数器
...
纯指标统计使用普通原子整数。计数值本身属于并发状态时，例如活跃
任务数量或终止判断，使用 `AtomicCounter`。
...
| 方法 | `AtomicCounter` | `AtomicSignedCounter` | 描述 |
|-----|-----------------|-----------------------|------|
| `new(value)` | `usize` | `isize` | 创建计数器 |
| `zero()` | 支持 | 支持 | 创建零值计数器 |
...
| `add(delta)` | 溢出 panic | 溢出 panic | 加 delta，返回新值 |
| `sub(delta)` | 下溢 panic | 溢出 panic | 减 delta，返回新值 |
| `try_add(delta)` | 溢出返回 `None` | 溢出返回 `None` | 检查式加法 |
| `try_dec()` | 零值返回 `None` | 不支持 | 检查式减一 |
| `try_sub(delta)` | 下溢返回 `None` | 溢出返回 `None` | 检查式减法 |
...
| **同步计数器** (`inc()`、`dec()`) | CAS 循环 | 计数值用于表达状态信号 |
...
|-----|-----|---------------|------|
| **基础类型** | 3 种类型 | 15 种类型 | Rust 支持更多整数和计数器类型 |
...
| **同步计数器** | 手动组合 | `AtomicCounter` | 用于状态跟踪的非负计数器 |
