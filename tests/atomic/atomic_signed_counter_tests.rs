...
#[test]
fn test_new_get() {
    let counter = AtomicSignedCounter::new(-42);
...
#[test]
fn test_default() {
    let counter = AtomicSignedCounter::default();
    assert_eq!(counter.get(), 0);
}

#[test]
fn test_from() {
    let counter = AtomicSignedCounter::from(-9);
    assert_eq!(counter.get(), -9);
}

#[test]
fn test_sign_checks() {
    let positive = AtomicSignedCounter::new(1);
    let zero = AtomicSignedCounter::zero();
    let negative = AtomicSignedCounter::new(-1);
...
fn test_try_add_overflow_keeps_value() {
    let counter = AtomicSignedCounter::new(isize::MAX);
...
fn test_try_add_underflow_keeps_value() {
    let counter = AtomicSignedCounter::new(isize::MIN);
...
fn test_debug() {
    ...
    assert_eq!(format!("{counter:?}"), "AtomicSignedCounter { value: -42 }");
}
...
fn test_concurrent_add() {
    ...
    let counter = Arc::new(AtomicSignedCounter::zero());
