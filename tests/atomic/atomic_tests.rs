...
#[test]
fn test_update_and_get_retry_paths() {
    let bool_atomic = Atomic::<bool>::new(false);
    let bool_raced = Cell::new(false);
    let bool_new = bool_atomic.update_and_get(|current| {
        if !bool_raced.replace(true) {
            bool_atomic.store(!current);
        }
        !current
    ...
    let int_atomic = Atomic::<i32>::new(1);
    let int_raced = Cell::new(false);
    let int_new = int_atomic.update_and_get(|current| {
        if !int_raced.replace(true) {
            int_atomic.store(current + 10);
        }
        current * 2
    });
    assert_eq!(int_new, 22);
    assert_eq!(int_atomic.load(), 22);

    let float_atomic = Atomic::<f32>::new(1.0);
    let float_raced = Cell::new(false);
    let float_new = float_atomic.update_and_get(|current| {
        if !float_raced.replace(true) {
            float_atomic.store(current + 10.0);
        }
        current * 2.0
    });
    assert_eq!(float_new, 22.0);
    assert_eq!(float_atomic.load(), 22.0);

    let double_atomic = Atomic::<f64>::new(1.0);
    let double_raced = Cell::new(false);
    let double_new = double_atomic.update_and_get(|current| {
        if !double_raced.replace(true) {
            double_atomic.store(current + 10.0);
        }
        current * 2.0
    });
    assert_eq!(double_new, 22.0);
    assert_eq!(double_atomic.load(), 22.0);
}
...
#[test]
fn test_try_update_and_get_success_and_reject_paths() {
    let bool_atomic = Atomic::<bool>::new(false);
    assert_eq!(
        bool_atomic.try_update_and_get(|current| (!current).then_some(true)),
        Some(true),
    );
    assert!(bool_atomic.load());
    assert_eq!(
...
#[test]
fn test_try_update_and_get_retry_paths() {
    let bool_atomic = Atomic::<bool>::new(false);
    let bool_raced = Cell::new(false);
    let bool_new = bool_atomic.try_update_and_get(|current| {
        if !bool_raced.replace(true) {
            bool_atomic.store(!current);
        }
        Some(!current)
    });
    assert_eq!(bool_new, Some(false));
    assert!(!bool_atomic.load());

    let int_atomic = Atomic::<i32>::new(1);
    let int_raced = Cell::new(false);
    let int_new = int_atomic.try_update_and_get(|current| {
        if !int_raced.replace(true) {
            int_atomic.store(current + 10);
        }
        Some(current * 2)
    });
    assert_eq!(int_new, Some(22));
    assert_eq!(int_atomic.load(), 22);

    let float_atomic = Atomic::<f32>::new(1.0);
    let float_raced = Cell::new(false);
    let float_new = float_atomic.try_update_and_get(|current| {
        if !float_raced.replace(true) {
            float_atomic.store(current + 10.0);
        }
        Some(current * 2.0)
    });
    assert_eq!(float_new, Some(22.0));
    assert_eq!(float_atomic.load(), 22.0);

    let double_atomic = Atomic::<f64>::new(1.0);
    let double_raced = Cell::new(false);
    let double_new = double_atomic.try_update_and_get(|current| {
        if !double_raced.replace(true) {
            double_atomic.store(current + 10.0);
        }
        Some(current * 2.0)
    });
    assert_eq!(double_new, Some(22.0));
    assert_eq!(double_atomic.load(), 22.0);
}
